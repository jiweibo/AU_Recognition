...
res50_model_name = r'resnet50-19c8e357.pth'
res101_model_name = r'resnet101-5d3b4d8f.pth'
...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        torch_home = os.path.expanduser(os.getenv('TORCH_HOME', '~/.torch'))
        model_dir = os.getenv('TORCH_MODEL_ZOO', os.path.join(torch_home, 'models'))
        model_path = os.path.join(model_dir, res50_model_name)
        model_param = torch.load(model_path)
        for name, value in model_param.items():
            if name.startswith('fc'):
                continue
...
def resnet101(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        torch_home = os.path.expanduser(os.getenv('TORCH_HOME', '~/.torch'))
        model_dir = os.getenv('TORCH_MODEL_ZOO', os.path.join(torch_home, 'models'))
        model_path = os.path.join(model_dir, res101_model_name)
        model_param = torch.load(model_path)
        for name, value in model_param.items():
            if name.startswith('fc'):
                continue
