...
vgg16_model_name = r'vgg16-397923af.pth'
vgg16_bn_model_name = r'vgg16_bn-6c64b313.pth'
vgg19_model_name = r'vgg19-dcbb9e9d.pth'
...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=512 * 7 * 7):
...
def vgg16(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model_path = os.path.join(model_dir, vgg16_model_name)
...
def vgg16_bn(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        torch_home = os.path.expanduser(os.getenv('TORCH_HOME', '~/.torch'))
        model_dir = os.getenv('TORCH_MODEL_ZOO', os.path.join(torch_home, 'models'))
        model_path = os.path.join(model_dir, vgg16_bn_model_name)
        model_param = torch.load(model_path)
        for name, value in model_param.items():
            if name.startswith('features') and name in model.state_dict():
                model.state_dict()[name].copy_(value)
...
def vgg19(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        torch_home = os.path.expanduser(os.getenv('TORCH_HOME', '~/.torch'))
        model_dir = os.getenv('TORCH_MODEL_ZOO', os.path.join(torch_home, 'models'))
        model_path = os.path.join(model_dir, vgg19_model_name)
        model_param = torch.load(model_path)
        for name, value in model_param.items():
            if name.startswith('features') and name in model.state_dict():
                model.state_dict()[name].copy_(value)
